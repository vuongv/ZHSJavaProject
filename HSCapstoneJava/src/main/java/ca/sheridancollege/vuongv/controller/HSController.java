...
public class HSController {
	...
	public String addOrder_POST(Model model,
			...
			@RequestParam String customerHomePhone, @RequestParam String customerCellPhone,
			@RequestParam String customerAddress, @RequestParam String customerCity,
			@RequestParam String customerPostal, @RequestParam String customerProvince,
			@RequestParam String orderServiceType, @RequestParam String orderWorker, @RequestParam String orderTotal,
		...
		Customer cust = Customer.builder().name(customerName).email(customerEmail).homePhone(customerHomePhone)
				.cellPhone(customerCellPhone).address(customerAddress).city(customerCity).postal(customerPostal)
				.province(customerProvince).workOrders(new ArrayList<WorkOrder>()).build();
		WorkOrder workOrder = WorkOrder.builder().orderDate(LocalDate.now()).appointmentDate(orderAppointmentDate)
				.appointmentTime(orderAppointmentTime).orderCost(new BigDecimal(orderTotal)).worker(orderWorker)
				.service(orderServiceType).status("Not Started").build();

		cust.getWorkOrders().add(workOrder);
		WorkOrder w = orderRepo.save(workOrder); 
	...
	public String editOrder(Model model, @PathVariable Long workOrderId, @PathVariable String customerId) {
		Optional<WorkOrder> order = orderRepo.findById(Long.valueOf(workOrderId));
	...
	public String editOrder(Model model, @PathVariable Long workOrderId, @PathVariable String customerId,
		...
		WorkOrder order = WorkOrder.builder().workOrderId(workOrderId).orderDate(orderDate)
				.appointmentDate(orderAppointmentDate).appointmentTime(orderAppointmentTime)
				.orderCost(new BigDecimal(orderTotal)).worker(orderWorker).status(orderStatus).service(orderServiceType)
				.build();

		Customer cust = Customer.builder().id(Long.valueOf(customerId)).name(customerName).email(customerEmail)
				.homePhone(customerHomePhone).cellPhone(customerCellPhone).address(customerAddress).city(customerCity)
				.postal(customerPostal).province(customerProvince).workOrders(oldCust.get().getWorkOrders()).build();
	...
	public String editCustomer(Model model, @RequestParam String id, @RequestParam String name,
			@RequestParam String email, @RequestParam String homePhone, @RequestParam String cellPhone,
			@RequestParam String address, @RequestParam String city, @RequestParam String postal,
			@RequestParam String province, RedirectAttributes redirectAttributes) {
		Optional<Customer> oldCust = customerRepo.findById(Long.valueOf(id));
		Customer cust = Customer.builder().id(Long.valueOf(id)).name(name).email(email).homePhone(homePhone)
				.cellPhone(cellPhone).address(address).city(city).postal(postal).province(province)
				.workOrders(oldCust.get().getWorkOrders()).build();
		customerRepo.save(cust);
	...
	public String addCustomer(Model model, @RequestParam String name, @RequestParam String email,
			@RequestParam String homePhone, @RequestParam String cellPhone, @RequestParam String address,
			@RequestParam String city, @RequestParam String postal, @RequestParam String province,
			RedirectAttributes redirectAttributes) {
		Boolean addSuccess;

		Customer cust = Customer.builder().name(name).email(email).homePhone(homePhone).cellPhone(cellPhone)
				.address(address).city(city).postal(postal).province(province).workOrders(new ArrayList<WorkOrder>())
				.build();
	...
	public String addService_POST(Model model, @RequestParam String serviceName, @RequestParam double serviceCost,
			@RequestParam String serviceDescription, @RequestParam double serviceDuration,
			RedirectAttributes redirectAttributes) {
		...
		WorkService service = WorkService.builder().serviceName(serviceName).serviceCost(serviceCost)
				.serviceDescription(serviceDescription).serviceDuration(serviceDuration).build();
	...
	public String editService(Model model, @RequestParam String serviceId, @RequestParam String serviceName,
			@RequestParam double serviceCost, @RequestParam String serviceDescription,
		...
		WorkService service = WorkService.builder().serviceId(Long.valueOf(serviceId)).serviceName(serviceName)
				.serviceCost(serviceCost).serviceDuration(serviceDuration).serviceDescription(serviceDescription)
				.build();
	...
	public String addWorker_POST(Model model, @RequestParam String name) {
		...
		WorkWorker worker = WorkWorker.builder().name(name).build();
	...
	public String addTestimonial(Model model, @RequestParam String orderServiceType, @RequestParam String email,
			@RequestParam String name, @RequestParam String message) {

		Testimonial t = Testimonial.builder().serviceName(orderServiceType).userEmail(email).userName(name)
				.userTestimonial(message).build();
	...
	public String addTestimonial_POST(Model model, @RequestParam String orderServiceType,
			@RequestParam String userEmail, @RequestParam String userName, @RequestParam String testimonial,
			RedirectAttributes redirectAttributes) {

		Testimonial t = Testimonial.builder().userName(userName).serviceName(orderServiceType).userEmail(userEmail)
				.userTestimonial(testimonial).build();
	...
	public String editTestimonial(Model model, @PathVariable String testimonialId,
			@RequestParam String orderServiceType, @RequestParam String userEmail, @RequestParam String userName,
			@RequestParam String testimonial, @RequestParam String toDisplay, RedirectAttributes redirectAttributes) {
		Testimonial test = Testimonial.builder().testimonialId(Long.valueOf(testimonialId)).userName(userName)
				.serviceName(orderServiceType).userEmail(userEmail).userTestimonial(testimonial)
				.toDisplay(Boolean.valueOf(toDisplay)).build();
	...
	public String addImage_POST(Model model, @RequestParam("image") MultipartFile multipartFile) throws IOException {
		if (imageRepo.existsByImageName(StringUtils.cleanPath(multipartFile.getOriginalFilename()))) {
			model.addAttribute("duplicate",
					"Sorry, this Image Name already exists, please enter a change the image name of your file");
			return "secure/addImage";
		}

		String fullPath = "/images/gallery/" + StringUtils.cleanPath(multipartFile.getOriginalFilename());
		Image i = Image.builder().imageName(StringUtils.cleanPath(multipartFile.getOriginalFilename()))
				.fullFilePath(fullPath).build();
	...
	public String deleteImage(Model model, @PathVariable String imageId, RedirectAttributes redirectAttributes) {
		Optional<Image> i = imageRepo.findById(Long.valueOf(imageId));
		try {
			Path path1 = Paths.get(i.get().getImageName());
			String dir = System.getProperty("user.dir") + "/src/main/resources/static" + i.get().getFullFilePath();
	...
	public String editImage(Model model, @PathVariable String imageId, RedirectAttributes redirectAttributes) {
	...
	public String editImage(Model model, @PathVariable String imageId, @RequestParam String imageName,
			@RequestParam String toDisplay, RedirectAttributes redirectAttributes) {
		...
		Image newImage = Image.builder().id(Long.valueOf(imageId)).imageName(imageName)
				.fullFilePath("/images/gallery/" + imageName).toDisplay(Boolean.valueOf(toDisplay)).build();
		String newDir = System.getProperty("user.dir") + "/src/main/resources/static" + newImage.getFullFilePath();
